...
(function () {
  ...
  function setVisual(scene) {
    ...
    if (scene.video) {
      ...
      var fit = scene.videoFit || "cover";
      var vid = '<video class="vid-main" src="' + scene.video + '" autoplay muted loop playsinline style="object-fit:' + fit + '"></video>';
      if (fit === "contain") {   // fill the side bars with a blurred copy
        vid = '<video class="vid-bg" src="' + scene.video + '" autoplay muted loop playsinline aria-hidden="true"></video>' + vid;
      }
      fig.innerHTML = vid;
    } else if (scene.image) {
      fig.classList.add("has-image");
